...
def get_merchant_categories(conn):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS merchant_categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL)")
...
def merchants_list():
    ...
    return render_template('merchants.html', merchants=merchants, treasuries=treasuries,
                           ...
                           categories=[c['name'] for c in merchant_cats], active_page='merchants')
...
def reports_view():
    ...
    # Build metrics dict compatible with template variable names
    metrics = dict(stats)
    ...
    status_breakdown = {
        ...
        'assigned_count': 0,
        'arrived_count': 0,
        'out_count': stats.get('out_orders', 0),
        'delivered_count': stats.get('delivered_orders', 0),
        'returned_count': 0,
        'partial_returned_count': 0,
        'cancelled_count': 0,
        'postponed_count': 0,
    }
    try:
