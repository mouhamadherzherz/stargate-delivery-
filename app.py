...
app.secret_key = os.environ.get('STARGATE_SECRET_KEY') or 'stargate-delivery-super-secret-key-2026'
...
def check_and_restore_gdrive_on_startup(sync_mode=False):
    """If running on Render / Cloud and database is empty, attempt auto-restore from Google Drive."""
    try:
        # Check environment variables first (no DB required)
        creds = (os.environ.get('GDRIVE_SERVICE_ACCOUNT_JSON') or '').strip()
        folder_id = (os.environ.get('GDRIVE_FOLDER_ID') or '').strip()

        # If not in env, check DB settings
        if not creds and os.path.exists(DB_PATH):
            try:
                conn = get_db()
                row = conn.execute("SELECT gdrive_credentials_json, gdrive_folder_id FROM settings WHERE id=1").fetchone()
                conn.close()
                if row:
                    creds = (row['gdrive_credentials_json'] or '').strip()
                    folder_id = (row['gdrive_folder_id'] or '').strip()
            except Exception:
                pass
        ...
        if order_count == 0:
            print("[Stargate] Database is empty. Attempting cloud auto-restore from Google Drive...")
            import google_drive_backup
            success, msg, details = google_drive_backup.download_latest_backup_from_drive(creds, folder_id, DB_PATH)
            if success:
                print(f"[Stargate] SUCCESS: Auto-restored cloud database on startup: {msg}")
                return True
            else:
                print(f"[Stargate] Cloud restore notice: {msg}")
    except Exception as e:
        print(f"[Stargate] Startup restore warning: {e}")
    return False

# Attempt immediate synchronous restore if credentials in environment
if os.environ.get('GDRIVE_SERVICE_ACCOUNT_JSON'):
    check_and_restore_gdrive_on_startup(sync_mode=True)
else:
