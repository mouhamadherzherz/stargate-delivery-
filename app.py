...
# نظام إدارة شركة دليفري متكامل ومعدل بالكامل لبيئات السحاب والتشغيل المحلي
# ==============================================================================
...
def verify_admin_pin(pin):
    ...
    return (pin_str == stored_pin or hash_pin(pin_str) == stored_pin)
    ...
    if request.method == 'POST':
        ...
        if login_type == 'pin':
            ...
            if emp:
                ...
                conn.close()
                flash("رمز PIN غير صحيح أو الحساب غير نشط!", "danger")
            return render_template('login.html')
        else:
...
def add_employee():
    ...
    try:
        cur.execute("""
        INSERT INTO employees (username, password_hash, display_name, role, phone, is_active, pin, custom_permissions)
        VALUES (?, ?, ?, ?, ?, 1, ?, ?)
        """, (username, hash_password(password), display_name, role, phone, pin, custom_permissions))
        conn.commit()
...
def edit_employee(emp_id):
    ...
    try:
        if new_password:
            cur.execute("""
            UPDATE employees SET display_name=?, role=?, phone=?, pin=?, custom_permissions=?, password_hash=?
            WHERE id=?
            """, (display_name, role, phone, pin, custom_permissions, hash_password(new_password), emp_id))
        else:
            cur.execute("""
            UPDATE employees SET display_name=?, role=?, phone=?, pin=?, custom_permissions=?
            WHERE id=?
            """, (display_name, role, phone, pin, custom_permissions, emp_id))
        conn.commit()
...
def delete_order(order_id):
    ...
    try:
        ...
        conn.commit()
        flash("تم حذف الأوردر وتسوية الأرصدة بنجاح 🗑️", "info")
    finally:
        conn.close()
    return redirect(url_for('orders_list'))
...
def api_whatsapp_send():
    ...
    try:
        ...
        if not phone or not message:
            return jsonify({'success': False, 'message': 'رقم الهاتف أو نص الرسالة مفقود'}), 400
        ...
        if provider == 'ultramsg':
            ...
            if not instance or not token:
                return jsonify({'success': False, 'message': 'بيانات UltraMsg غير مكتملة'}), 400
            
            import urllib.request
            url = f"https://api.ultramsg.com/{instance}/messages/chat"
            payload = urllib.parse.urlencode({'token': token, 'to': phone, 'body': message}).encode('utf-8')
            req = urllib.request.Request(url, data=payload, method='POST')
            req.add_header('Content-Type', 'application/x-www-form-urlencoded')
            with urllib.request.urlopen(req, timeout=10) as resp:
                result = json.loads(resp.read().decode('utf-8'))
            if result.get('sent') == 'true' or result.get('id'):
                return jsonify({'success': True, 'message': f'تم إرسال الرسالة إلى {phone} بنجاح ✅'})
            return jsonify({'success': False, 'message': f'استجابة المزود: {result}'})
        else:
...
def trigger_gdrive_sync_async(delay=6.0):
    ...
    with _gdrive_sync_lock:
        ...
        def _do_upload():
            try:
                ...
                if enabled and creds and folder_id:
...
def _auto_sync_mutations_to_gdrive(response):
    try:
        if request.method == 'POST' and response.status_code in (200, 201, 302):
            ...
            if any(k in path for k in ['/order', '/merchant', '/courier', '/treasur', '/settle', '/expense']):
                if not any(ex in path for ex in ['/search', '/test', '/filter', '/export', '/report']):
                    trigger_gdrive_sync_async(delay=6.0)
    except Exception:
        pass
    return response
...
def check_and_restore_gdrive_on_startup():
    ...
    try:
        ...
        if order_count == 0 and creds and folder_id:
        ...
        print(f"[Stargate] Startup restore notice: {e}")
...
def api_live_pulse():
    try:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*), COALESCE(MAX(id), 0), COALESCE(MAX(delivered_at), ''), COALESCE(MAX(created_at), '') FROM orders")
        o_row = cursor.fetchone()
        cursor.execute("SELECT COUNT(*), COALESCE(MAX(id), 0) FROM couriers")
        c_row = cursor.fetchone()
        cursor.execute("SELECT COUNT(*), COALESCE(MAX(id), 0) FROM treasuries")
        t_row = cursor.fetchone()
        conn.close()

        fingerprint = f"{o_row[0]}:{o_row[1]}:{o_row[2]}:{c_row[0]}:{t_row[0]}"
        return jsonify({'success': True, 'fingerprint': fingerprint, 'timestamp': datetime.now().strftime('%H:%M:%S')})
    except Exception as e:
