...
def employees_list():
    ...
    params = []
    if q:
        query += " WHERE username LIKE ? OR display_name LIKE ? OR phone LIKE ?"
        params = [f"%{q}%", f"%{q}%", f"%{q}%"]
...
def add_employee():
    ...
    hashed_pin = hash_pin(pin) if pin else None
...
def delete_employee(emp_id):
    ...
    if emp['username'] in ('stargate', 'admin'):
        conn.close()
        flash("لا يمكن حذف حساب المدير الرئيسي النظام!", "danger")
        return redirect(url_for('employees_list'))

    cursor.execute("DELETE FROM employees WHERE id = ?", (emp_id,))
    log_audit(cursor, 'delete', 'employee', emp_id, f"username={emp['username']}")
    conn.commit()
    conn.close()
    flash(f"تم حذف الموظف [{emp['display_name']}] بنجاح 🗑️", "info")
    return redirect(url_for('employees_list'))
